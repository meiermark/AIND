...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        def check_cond(conditions, action_preconds):
            for precond in action_preconds:
                if precond not in conditions:
                    return False
            return True

        for action in self.get_actions():
            if (check_cond(cond_pos, action.precond_pos) and
                 check_cond(cond_neg, action.precond_neg)):
    ...
    def result(self, state: str, action: Action):
        ...
        for possible_action in self.actions(state):
            if (action.name == possible_action.name and
                action.args == possible_action.args):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        def check_goal_achieved(pos_states, goals):
            for state in goals:
        ...
        goal_achieved = check_goal_achieved(pos_states, self.goal)
        for action in actions:
            if not goal_achieved:
                relevant_pos_effects = [effect for effect in action.effect_add
                                                if effect in self.goal and
                                                effect not in pos_states]
